...
func createPipeline(ctx context.Context, logger *zap.Logger, registry *engine.Registry, injector do.Injector, job v1.CollectJob) (*engine.Pipeline, error) {
	logger.Info("creating pipeline", zap.String("job_name", job.Metadata.Name))
	spec := job.Spec
	pipeline := engine.NewPipeline(job.Metadata.Name)

	// Create collectors using the registry
	for _, collectorSpec := range spec.Collectors {
		resolved, err := ResolveCollectorSpec(collectorSpec)
		if err != nil {
			return nil, fmt.Errorf("failed to resolve collector %s: %w", collectorSpec.ID, err)
		}

		collector, err := registry.CreateCollector(ctx, injector, resolved.Kind, resolved.Spec)
		if err != nil {
			return nil, fmt.Errorf("failed to create collector %s: %w", collectorSpec.ID, err)
		}

		if err := pipeline.AddCollector(collectorSpec.ID, collector); err != nil {
			return nil, fmt.Errorf("failed to add collector %s: %w", collectorSpec.ID, err)
		}

		logger.Info("created collector", zap.String("collector_id", collectorSpec.ID), zap.String("kind", resolved.Kind))
	}

	// Create steps using the registry
	for _, stepSpec := range spec.Steps {
		resolved, err := ResolveStepSpec(stepSpec)
		if err != nil {
			return nil, fmt.Errorf("failed to resolve step %s: %w", stepSpec.ID, err)
		}

		// Get the collector if the step requires one
		var collector engine.Collector
		if stepSpec.Collector != nil {
			var ok bool
			collector, ok = pipeline.GetCollector(*stepSpec.Collector)
			if !ok {
				return nil, fmt.Errorf("step %s references unknown collector %s", stepSpec.ID, *stepSpec.Collector)
			}
		}

		step, err := registry.CreateStep(ctx, injector, resolved.Kind, collector, resolved.Spec)
		if err != nil {
			return nil, fmt.Errorf("failed to create step %s: %w", stepSpec.ID, err)
		}

		if err := pipeline.AddStep(stepSpec.ID, step); err != nil {
			return nil, fmt.Errorf("failed to add step %s: %w", stepSpec.ID, err)
		}

		logger.Info("created step", zap.String("step_id", stepSpec.ID), zap.String("kind", resolved.Kind))
	}
