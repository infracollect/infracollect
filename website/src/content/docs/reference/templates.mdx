...
Additional environment variables can be made available using the `--allowed-env` flag:

```bash
infracollect collect job.yaml --allowed-env AWS_REGION --allowed-env ENVIRONMENT
```

Only explicitly allowed environment variables can be referenced. If an allowed variable is not set in the environment, the job will fail.
...
```bash
infracollect collect job.yaml --allowed-env API_USER --allowed-env API_PASSWORD
```
